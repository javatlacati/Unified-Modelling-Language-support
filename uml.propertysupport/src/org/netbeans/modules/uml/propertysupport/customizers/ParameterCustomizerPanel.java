...
public class ParameterCustomizerPanel extends javax.swing.JPanel {
        ...
        if (dataList != null) {
            ...
            if (parentElement != null) {
                ...
                if (paramOrderChanged && dataList.length > 1)
                {
                    ...
                    List < IPropertyElement > children = parentElement.getSubElements();
    ...
    protected void save(IPropertyElement element) {
        if(element != null) {
            element.save();
            List < IPropertyElement > children = element.getSubElements();
            for(IPropertyElement child : children) {
                save(child);
            }
        }
        ...
        public void addRow() {
            ...
            if (newElement != null) {
                if (rowsData != null) {
                    ...
                    List <IPropertyElement> rowCells = newElement.getSubElements();
                    if (rowCells != null && rowCells.size() >= 2) {
                        rowCells.get(0).setValue("0");
                        rowCells.get(1).setValue("*");
                    }
