...
public class VersionableElement implements IVersionableElement {
    ...
    /**
     * Retrieves the XML node associated with this element. HRESULT Node([out,
     * retval] IXMLDOMNode* *pVal);
     */
    @Override
    public Node getNode() {
        return m_Node;
    ...
    /**
     * Sets the XML node associated with this element. HRESULT Node([in]
     * IXMLDOMNode* newVal);
     */
    @Override
    ...
    /**
     * Initializes the internal XML node. HRESULT PrepareNode( [ in,
     * defaultvalue(0)] IXMLDOMNode* parentNode );
     */
    @Override
    ...
    /**
     * This is not implemented here. This is implemented in sub objects of this
     * type.
     */
     ...
     * element is to be versioned when saved next. This is a temporary state of
     * this element, as the VersionMe flag can only exist on elements that have
     * not previously been versioned. Once done, this call, as well as the
     * put_VersionMe(), have no effect.
     *
     * @param pVal[out] The VersionMe flag. true indicates that this element
     * needs to be versioned. false means that the element does not need to be
     * versioned.
     *
     * @return HRESULTs
    ...
    /**
     * Sets the "versionMe" XML attribute on this element's DOM node. However,
     * this operation will only occur if this element has not previously been
     * versioned. If the element has been versioned, this is a noop. If the flag
     * is set, it acts as a dirty flag as well, indicating to the system that
     * this element needs to be saved.
     *
     * @param newVal[in] The flag indicating whether or not this element is to
     * be versioned
     *
     * @return HRESULTs - S_FALSE The flag was not set because the element has
     * already been versioned. - S_OK Flag was set - E_INVALIDARG m_Node has not
     * been established
     */
    @Override
     ...
     * "isDirty" XML attribute is injected into the DOM node and set to the
     * value dictated by newVal. If this element has not been versioned, or is
     * not marked to be versioned, the owning document is marked as dirty.
     *
    ...
    /**
     * Sets the XML id of this node. Right now the only one using this is the
     * gui when a presentation object is created and needs to be set back to its
     * original id when the graphical node was first created.
     *
     * @param newVal[in]
     *
     * @result S_OK
    ...
    /**
     *
     * Determines whether or not this element is the same as the passed in
     * element. The comparison is made between the XMI ids.
     *
     * @param element[in] The element to check against this one. If 0, result
     * will equal false.
     ...
     *
     * @see
     * #isSame(Lorg/netbeans/modules/uml/core/metamodel/core/foundation/IVersionableElement)
     *
     */
    @Override
    ...
    /**
     *
     * Deletes this element from the existing document. All references to the
     * element are removed.
     *
     * @return HRESULT
     *
    ...
    /**
     *
     * Before performing the actual delete, this method makes sure that no other
     * object references this element. If there is a reference, the element is
     * not deleted.
     *
     *
     * @return HRESULT
     ...
     * guaranteed to work even for ITransitionElement instances, and for
     * IElements that are children of ITransitionElements.
     *
     ...
     *
     * @param affectedElements[in] Elements that will be affected by this
     * deletion. If 0 is passed, the elements will be calculated.
     *
     * @return HRESULT
     *
     */
     ...
     * @param ver[in] The COM object that represents this impl instance
     *
     * @return - true if the deletion of this element should occur, else - false
     * if not
     *
     */
    ...
    /**
     *
     * Retrieves the file name that this element is housed in. This will only be
     * valid if this element has been versioned.
     *
    ...
    /**
     * The name of the file that this element is versioned in. If the element
     * has not been versioned, this value is ignored. HRESULT
     * VersionedFileName([in] BSTR newVal);
     */
    @Override
    ...
    /**
     *
     * Saves the contents of this element out to the etx file if this element is
     * indeed versioned. If the element is not dirty, then the only thing that
     * will happen as a result of this call is that the element will be unloaded
     * from memory
     *
     * @param bSaved[out] - true if the element was saved, else - false. It will
     * return false if the element didn't need to be saved.
     *
     * @return HRESULT
     *
     */
    ...
    /**
     *
     * Duplicates this element. The duplicate element is the same as the current
     * element except that it will be assigned a new XMI id and removed from the
     * Namespace that the current element is in. Also, if the current element is
     * version controlled, the duplicate element will NOT have the version
     * control information established.
     *
    ...
    /**
     *
     * This routine returns the URI to this element, if the element has been
     * extracted ( most likely due to version control ) from the Project's DOM.
     * This routine is smart in that if this element is encapsulated by an
     * element that was extracted, that element's URI is used to build this
     * ones.
     *
     * @param uri[out] The URI of this object. If it is determined that this
     * element or any of its parents has not been extracted, then the XMI id of
    ...
    /**
     *
     * Removes all version control information from this element. Once this is
     * done, the element will appear as if it had never been versioned.
     *
     * @return HRESULT
     *
     */
     ...
     * the raw xmi id. The element will also not be a part of the type file once
     * done
     *
    ...
    /**
     * Verifies that this element is properly represented in memory. During
     * version control processing, it is possible for an element to be
     * 'orphaned' in memory, as the parent elements of this element could have
     * been unloaded from memory, but due to a client holding on to this object,
     * this object remains. This method ensures that proper object ownership is
     * maintained.
     *
     * @param wasModified[out] - true if the xml representation behind this
     * element has been changed as a result of verification, else - false if not
     *
     * @return HRESULT
     */
    @Override
    ...
    /**
     * Retrieves the line number associated with this element. HRESULT
     * LineNumber([out, retval] long * lineNumber );
     */
    @Override
    ...
    /**
     * Sets the line number associated with this element. HRESULT
     * LineNumber([in] long lineNumber );
     */
    @Override
    ...
    /**
     * Creates the actual XML node of the passed in type and establishes the
     * created element as a child of the passed in parent node. All elements
     * that wish to participate in the persistence mechanism must call this
     * method.
     *
    ...
    protected IVersionableElement performDuplication() {
        ...
        if (clone != null) {
            ...
            if (clonedNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                boolean isVersioned = clone.isVersioned();
                if (isVersioned) {
    ...
    /**
     *
     * Does an insert of the nodes coming in. This is a generic method that
     * assumes that existingVer is currently owned in some way by the current
     * VersionableElement, and that newVer is being inserted into the same
     * collection that existingVer is a part of.
     *
     * @param owningElement[in] The actual xml element that is the direct owner
     * of the elements. If this parameter is 0, the current element will be
     * queried for the "UML:Element.ownedElement" element, and that will be used
     * as the owning element.
...
//	}
     ...
     * will contain the COM object representing this impl class
     *
     * @return HRESULT
     *
     */
