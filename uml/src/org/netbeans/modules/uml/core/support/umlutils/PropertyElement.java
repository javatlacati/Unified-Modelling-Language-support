...
public class PropertyElement implements IPropertyElement{
  ...
  //< IPropertyElement[] >
  private List<IPropertyElement> m_subElements = new Vector<>();
...
//      }
  ...
  @Override
  public List<IPropertyElement> getSubElements() {
  ...
  @Override
  public void setSubElements( List<IPropertyElement> value ) 
  {
  	m_subElements.clear();
    ...
    if (value != null)
    {
        for (IPropertyElement ele : value) {
  ...
  {
    int pos = m_subElements.indexOf(element); //TODO replace for a colection that starts looking from specified index, index
    if (pos >= 0)
      return element;
    return null;
  }
