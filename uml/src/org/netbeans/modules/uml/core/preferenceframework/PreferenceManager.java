...
public class PreferenceManager implements IPreferenceManager2{
	...
	public void setPropertyDefinitions(IPropertyDefinition[] value) 
	{
		if (value != null)
		{
			...
			for (IPropertyDefinition iPropertyDefinition : value) {
				m_Definitions.add(iPropertyDefinition);
			}
		}
	}
	...
	public void setPropertyElements(IPropertyElement[] value) 
	{
		...
		if (value != null)
		{
			for (IPropertyElement elem : value) {
				m_Elements.add(elem);
			}
		}
	}
	...
	private void buildElements(String dataFile) {
		if (m_EleManager != null)
		{
			...
			int eleCount = 0;
			for (IPropertyDefinition def : m_Definitions) {
				IPropertyElement[] elems = m_EleManager.buildElementsUsingXMLFile(def);
				if (elems != null) {
					for (IPropertyElement elem : elems) {
						m_Elements.add(elem);
						eleCount++;
					}
				}
 ...
 */
	...
	private void buildNecessaryStructures(String key, String path, IPropertyDefinition pDef, IPropertyElement pEle) {
		...
		if (rootDef != null && topEle != null)
		{
			...
			IPropertyDefinition prevDef;
			IPropertyElement prevEle;
			...
			if (path.length() > 0)
			{
				...
				if (strs != null && strs.length > 0)
				{
					for (String str1 : strs) {
						...
						IPropertyDefinition def = getDefinition(prevDef, str1);
						IPropertyElement ele = getElement(prevEle, str1);
	...
	private void removePreference(IPropertyElement elem, IPropertyElement pEle) {
		String pathToRemove = pEle.getPath();
		List subElems = elem.getSubElements();
		if (subElems != null)
		{
...
//		return elems;
	...
	private boolean saveSubElements(Document doc, IPropertyElementXML ele) {
		...
		List<IPropertyElement> elems = ele.getSubElements();
		if (elems != null && elems.size() > 0)
		{
			for (IPropertyElement obj : elems) {
				if (obj instanceof IPropertyElementXML) {
			...
			if (path.length() > 0)
			{
				...
				if (path.contains("|"))
				{
					...
					if (strs != null && strs.length > 0)
					{
						for (String str : strs) {
							if (tempEle != null) {
								tempEle = getElement(tempEle, str);
							}
						}
	...
	private IPropertyElement getElement(IPropertyElement rootEle, String prefName) {
		...
		if (rootEle != null)
		{
			...
			{
				List elems = rootEle.getSubElements();
				if (elems != null)
				{
					for (int i=0; i<elems.size(); i++)
					{
						IPropertyElement elem = (IPropertyElement)elems.get(i);
		...
		if (rootDef != null)
		{
			if (path.length() > 0)
			{
				...
				if (strs != null && strs.length > 0)
				{
					for (String str : strs) {
						if (tempDef != null) {
							tempDef = getDefinition(tempDef, str);
						}
					}
	 ...
	 *
	 */
	private void restoreElements(List elems, boolean forInstall) {
		if (elems != null)
		{
	...
	public IPropertyElement findElement(IPropertyElement pEle, String sName) {
		...
		else
		{
			...
			if (subElems != null)
			{
				for(int i=0; i<subElems.size(); i++)
				{
					IPropertyElement elem = (IPropertyElement)subElems.get(i);
