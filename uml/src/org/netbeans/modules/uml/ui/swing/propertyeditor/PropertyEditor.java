...
//		m_DispatchHelper.registerDrawingAreaEvents(eventSink);
	...
	private void init()
	{
		...
		if (conMan != null)
		{
			String file = conMan.getDefaultConfigLocation();
...
//				helper.revokeDrawingAreaCompartmentSink(m_EventsSink);
	...
	{
		if (!PropertyEditorBlocker.inProcess() /*&& m_Model != null*/)
		{
	...
	public long loadElements(IElement[] pElements)
	{
		...
		if (!PropertyEditorBlocker.inProcess() && m_PropertyGrid != null)
		{
			processLastCell(true);
			clear();
			//
			...
			Vector<IPropertyElement> propElems = new Vector<>();
			Vector<IPropertyDefinition> propDefs = new Vector<>();
...
//		if (langs != null && langs.length > 0)
	...
	public void processLastCell(boolean elementChanged, int row)
	{
		if (m_Model != null && row != 0)
		{
			...
			IColorProperty pColorProperty = getColorPropertyAtGridRow(row);
			IFontProperty pFontProperty = getFontPropertyAtGridRow(row);
			JDefaultMutableTreeNode node = getNodeAtGridRow(row);
			IPropertyElement nodeEle = getElementAtGridRow(row);
...
//				}
			...
			if (values != null && values.length() > 0)
			{
				...
				else
				{
					...
					else
					{
						...
						IEnumTranslator enumTranslator = new EnumTranslator();
						String enumValues = pDef.getFromAttrMap("enumValues"); //$NON-NLS-1$
						int num = enumTranslator.translateToEnum(newVal, values, enumValues);
						if (num != -999)
						{
							pEle.setValue(Integer.toString(num));
						}
	...
	private void reorderFeatures()
	{
		...
		ETList<IParameter> pFeatures = new ETArrayList<>();
	...
	public Vector<Icon> loadImages(JDefaultMutableTreeNode root)
	{
		...
		if (root != null)
		{
			...
			if (obj != null && obj instanceof IPropertyElement)
			{
				...
				List<IPropertyElement> elems = rootEle.getSubElements();
				if (elems != null)
				{
					int count = elems.size();
					for (int i=0; i<count; i++)
					{
						IPropertyElement pEle = elems.get(i);
						IPropertyDefinition pDef = pEle.getPropertyDefinition();
						String name = pDef.getName();
		...
		if (pEle != null)
		{
			...
			if (pDef != null)
			{
				...
				if (m_ResourceMgr != null)
				{
					...
					else if ("Diagram".equals(name)) //$NON-NLS-1$
					{
						Object pDisp = pEle.getElement();
		...
		try {
			...
			else
			{
				...
				if (PropertyEditorBlocker.inProcess() == false)
				{
					...
					//
					Vector<IPropertyDefinition> propDefs = new Vector<>();
					Vector<IPropertyElement> propEles = new Vector<>();
...
//		}
	...
	{
		Vector<Object> retObj = new Vector<>();

		if (pEle != null)
		{		
			IPropertyDefinition pDef = pEle.getPropertyDefinition();
			List<IPropertyElement> elems = pEle.getSubElements();
			...
			if (elems != null)
			{
				...
				for (int i=0; i<count; i++)
				{
					...
					IPropertyElement subEle = elems.get(i);
...
//			}
			...
			switch (name) {
				case "FontProperty"://$NON-NLS-1$

					specialFontProcessing(newRow, pDef, pEle);
					break;
				case "ColorProperty"://$NON-NLS-1$

					specialColorProcessing(newRow, pDef, pEle);
					break;
				case "PropertyContainer"://$NON-NLS-1$
	...
	private void specialFontProcessing2(int row, IFontProperty fontProp, IPropertyElement pEle)
	{
		...
		if (subEles != null)
		{
			...
			{
				IPropertyElement subEle = subEles.get(i);
...
//			_VH(m_PropertyGrid->put_Cell(flexcpFontBold, CComVariant(row), CComVariant(m_DataCol), vtMissing, vtMissing, CComVariant(boldness)));
	...
	public void onPopupDelete()
	{
		if (m_Model != null && m_Tree != null)
		{
			...
			Vector<JDefaultMutableTreeNode> tempRows = new Vector<>();
			for (int i=0; i<count; i++)
			{
				int rowNum = rows[i];
				if (count == 1)
			...
			{
				JDefaultMutableTreeNode node = tempRows.elementAt(i);
		...
		if (m_Tree != null)
		{
			if (m_root != null)
			{
				...
				if (obj != null && obj instanceof IPropertyElement)
				{
					...
					rootEle.setSubElements(new Vector<>());
					...
					else
					{
						...
						List subElems = pEle.getSubElements();
...
//		m_EditableColor = GetSysColor(COLOR_WINDOW);
	...
	public void onPopupFill()
	{
		if (m_Model != null && m_Tree != null)
		{
			...
			try
			{
				...
				List<IPropertyElement> temp = new ArrayList<>();
	...
	{
		List<IElement> tempElements = new ArrayList<>();
		IPropertyElement pEle = getElementAtGridRow(m_CurRow);
		if (pEle != null)
		{
			Object pDisp = pEle.getElement();
		...
		if (m_ListBuffer == null || m_ListBuffer.length() == 0)
		{
			...
			try
			{
				...
				if (pDef != null)
				{
					...
					if (strs != null)
					{
						TreeMap <String, TreeMap<String, String> > theMap = new TreeMap<>();
						...
						for (int i=0; i<count; i++)
						{
							...
							TreeMap<String,String> inner = theMap.get(lower);
							if (inner != null)
							{
								String found = inner.get(full);
			...
			if (pNewPropEle != null)
			{
				...
				if (pPropEle != null)
				{
					...
					if (colEle != null)
					{
						...
						//
						Vector<IPropertyElement> temp = new Vector<>();
						List<IPropertyElement> subEles = colEle.getSubElements();
						int count = subEles.size();
						for (int i=count-1; i>=0; i--)
						{
							IPropertyElement subEle = subEles.get(i);
				...
				if (value == null && (text == null || text.length() == 0))
				{
					...
					if (recNode != null)
					{
						...
						while (recNode != null)
						{
							...
							if (insertNecessary(node, recNode) || !stopCellEditing)
							{
								insert(recNode);
							}
					...
					try
					{
						...
						if (obj != null)
						{
							...
							if (countMethod != null)
							{
								...
								{
									 int counter = (Integer) countResult;
									 if(counter > 0)
									 {
