...
public abstract class AbstractNBTask extends Thread
    ...
    public AbstractNBTask(HashMap<String, Object> settings) {
        taskSettings = settings;
        initialize();
    }

    public AbstractNBTask(HashMap<String, Object> settings, ITaskFinishListener listener) {
        taskSettings = settings;
        initialize();
        addListener(listener);
    }
    ...
    protected void finishLog() {
        ...
        if (cancelled) {
            log(NbBundle.getMessage(
                    AbstractNBTask.class, "MSG_Report_Cancelled") + " "
                    + // NOI18N
                    NbBundle.getMessage(
                            AbstractNBTask.class, "MSG_TotalTime", timeMsg)); // NOI18N
        } else if (success) {
            log(NbBundle.getMessage(
                    AbstractNBTask.class, "MSG_Report_Successful") + " "
                    + // NOI18N
                    NbBundle.getMessage(
                            AbstractNBTask.class, "MSG_TotalTime", timeMsg)); // NOI18N
        } else {
            log(NbBundle.getMessage(
                    AbstractNBTask.class, "MSG_Report_Failed") + " "
                    + // NOI18N
                    NbBundle.getMessage(
                            AbstractNBTask.class, "MSG_TotalTime", timeMsg)); // NOI18N
        }
    ...
    public boolean start(int contributor, int totalItems) {
        ...
        progressHandle.setDisplayName(getTaskName() + ": "
                + // NOI18N
    ...
    private void finishTask() {
        ...
        timeMsg = minutes == 0
                ...
                : minutes + " " + getBundleMessage("MSG_Minutes")
                + // NOI18N
                " " + seconds + " " + getBundleMessage("MSG_Seconds"); // NOI18N
