...
public class WizardRoles extends WizardInteriorPage {
    ...
    private void buildRoleMaps(IDesignPatternRole pRole) {
        if (pRole != null) {
            Hashtable <String, Vector<String> > eleNameToIDMap = new Hashtable < >();
            ...
            if (pRole != null) {
                ...
                if ( parentNode != null && !parentNode.equals(m_Root)) {
                    ...
                    if (parentNode.getChildCount() > 1) {
                        ...
                        // unique
                        Integer i = parentNode.getChildCount();
                        sName += i.toString();
                    }
                }
    ...
    public ETList<Integer> handleKeys(int row) {
        ETList<Integer> pKeys = new ETArrayList<>();
        ...
        boolean isMultInst = isMultipleParticipantInstance(row);
        if (isMult) {
            pKeys.add(java.awt.event.KeyEvent.VK_INSERT);
        } else if (isMultInst) {
            pKeys.add(java.awt.event.KeyEvent.VK_INSERT);
            pKeys.add(java.awt.event.KeyEvent.VK_DELETE);
        }
        return pKeys;
    }
    ...
    public String getIDForElementNamed(String name, WizardRoleObject obj) {
        ...
        if (obj != null) {
            ...
            if (pRole != null) {
                ...
                Map ht = m_TypeMap.get(type);
                if (ht != null) {
                    Object nameObj = ht.get(name);
                    if (nameObj != null) {
                        List names = (Vector)nameObj;
    ...
    private ETList<String> validatePage() {
        ETList<String> tempList = new ETArrayList<>();
        if (m_Wizard != null) {
            IDesignPatternDetails pDetails = m_Wizard.getDetails();
            if (pDetails != null) {
