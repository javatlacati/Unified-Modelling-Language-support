...
public class DesignPatternDetails implements IDesignPatternDetails {
    ...
    private Map< String, Vector<String>> m_ParticipantNameMap = new Hashtable<>();
    private Map< String, Vector<String>> m_ParticipantRoleMap = new Hashtable<>();
    private Map< String, Vector<IElement>> m_ParticipantInstanceMap = new Hashtable<>();
    ...
    /**
     * Whether or not a diagram should be created when a pattern is applied. The
     * diagram would contain the information that was created/changed during the
     * apply process.
     *
    ...
    /**
     * Whether or not to show the actual collaboration on the diagram that is
     * created when a pattern is applied
     *
     * @param[out] pVal	The flag for whether or not to create the collaboration
     * draw object
     *
     * @return HRESULT
     */
    ...
    /**
     * A map is kept for every participant in a pattern and what role that
     * participant is supposed to be playing.
     *
    ...
    /**
     * Get the entries from the map that match the passed in roleID. This will
     * be the names of the participants that will playing that role.
     *
    ...
    public ETList< String> getParticipantNames(String roleID) {
        ETList<String> pTemp = new ETArrayList<>();
        if (roleID != null && roleID.length() > 0) {
            List<String> pVals = m_ParticipantNameMap.get(roleID);
            if (pVals != null) {
    ...
    /**
     * A map is kept of the items playing in the pattern and the role that they
     * are playing
     *
    ...
    /**
     * Get the entries from the map that match the passed in id. This will be
     * the names of the participants that will playing that role.
     *
    ...
    public ETList< String> getParticipantRoles(String participantID) {
        ...
        if (participantID != null && participantID.length() > 0) {
            List<String> pVals = m_ParticipantRoleMap.get(participantID);
            if (pVals != null) {
                int count = pVals.size();
                for (int x = 0; x < count; x++) {
    ...
    /**
     * A map is kept of the roles in the pattern and the element that is playing
     * it
     *
    ...
    /**
     * Get the entries from the map that match the passed in role id. This will
     * be the elements that will playing that role.
     *
     * @param[in]	ID	The ID to find
    ...
    public ETList< IElement> getParticipantInstances(String roleID) {
        ETList<IElement> pTemp = new ETArrayList<>();
        if (roleID != null && roleID.length() > 0) {
            List<IElement> pVals = m_ParticipantInstanceMap.get(roleID);
            if (pVals != null) {
                int count = pVals.size();
                for (int x = 0; x < count; x++) {
    ...
    /**
     * The name of the diagram to create when a pattern is applied. The diagram
     * would contain the information that was created/changed during the apply
     * process.
     *
    ...
    /**
     * Whether or not a to remove a pattern from its current project when
     * promoting it to the design center area.
     *
