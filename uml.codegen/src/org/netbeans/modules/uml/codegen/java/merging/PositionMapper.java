...
{
   private TreeMap<Long, Long> positionMap = new TreeMap<>();
   ...
   {
      positionMap.put(position, bytesShift);
   }
   ...
   public long getMappedPositionFor(long position)
   {
      ...
      {
         posKey = iter.next();
         if (posKey != null && posKey <= position)
         {
   ...
   public String toString() {
      ...
      while (iter.hasNext()){
         ...
         strBuffer.append(" = ");
         strBuffer.append((positionMap.get(aKey)).longValue());
         strBuffer.append("\n");
      }
