...
/**
 * Create a fresh J2SEProject from scratch. Currently does not permit much to be
 * specified - feel free to add more parameters as needed.
 *
 * @author Mike Frisino
 */
...
//		return helper;
    ...
    public static AntProjectHelper createRevEngProject(File dir,
        ...
        ArrayList<File> reSrcsList = new ArrayList<>();
        ArrayList<String> reSrcsIdList = new ArrayList<>();

        for (int i = 0; i < rootsModel.getRowCount(); i++) {
            boolean includeSrcGrp
                    = partialRE
                    || ((Boolean) rootsModel.getValueAt(i,
                            JavaSourceRootsUI.COL_INCLUDE_FLAG));
    ...
    private static boolean hasInvalidCharacter(String name) {
        return replacements.entrySet().stream().anyMatch((e) -> (name.contains(e.getKey())));
    }
...
//        }
    ...
    public static String[] addJavaSrcProjRef(AntProjectHelper h,
        ...
        if (javaSrcArtiProv != null) {
            ...
            for (AntArtifact javaSrcAntArti : javaSrcAntArtis) {
                URI[] artiURIs = javaSrcAntArti.getArtifactLocations();
                for (URI artiURI : artiURIs) {
                    // AntArtifact artifact, URI location
                    refStrs.add(referenceHelper.addReference(javaSrcAntArti, artiURI));
                }
            }
    ...
    /**
     * Project reference ID cannot contain dot character. File reference can.
     */
    ...
    /**
     * Find reference ID (e.g. something you can then pass to RawReference as
     * foreignProjectName) for the given property base name, prefix and path.
     *
    ...
    /**
     * Generate unique reference ID for the given property base name, prefix and
     * path. See also {@link #findReferenceID(String, String, String)}.
     *
     * @param property project name or jar filename
     * @param prefix prefix used for reference, i.e. "project." for project
     * reference or "file.reference." for file reference
