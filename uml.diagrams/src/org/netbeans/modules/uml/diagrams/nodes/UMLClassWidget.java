...
//        addToLookup(new ClassifierSelectAction());
    ...
    protected boolean initializeClassView(IClassifier element) {
        ...
        if (classView == null) {
            ...
            if (classView.getFont() != null) {
                setFont(classView.getFont());//need to trigger font verification and update
            }
...
//            classView.addChild(new SeparatorWidget(scene, SeparatorWidget.Orientation.HORIZONTAL));
    ...
    protected void removeRedefinedOperation(IOperation op) {
        ElementListWidget redefinedOperations;
        List<IRedefinableElement> redefined = op.getRedefinedElements();
        for (IRedefinableElement element : redefined) {
            if (element instanceof IFeature) {
                IFeature feature = (IFeature) element;
...
//                                                   SeparatorWidget.Orientation.HORIZONTAL));
    ...
    public void propertyChange(PropertyChangeEvent event) {
        ...
        if (classView != null) {
            ...
            {
                if (newVal == null) {
                    newVal = oldVal;//it's in current moved event realization
                }
     ...
     * will be at TEMPLATE_EXTENDS and the size of the classifeir widget will be
     * the preferred size of the template parameter minus TEMPLATE_EXTENDS.
     */
    ...
    protected void notifyFontChanged(Font font) {
        ...
        if (nameWidget != null) {
            if (classView != null) {
                nameWidget.setNameFont(font);//it works in classview only
            }
        }
