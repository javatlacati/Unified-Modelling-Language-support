...
public class DesignerScene extends GraphScene<IPresentationElement, IPresentationElement> implements DiagramNodeWriter//, DiagramNodeReader 
{
     ...
     *
     * Since some operations require the list to be in the order of the
     * selection I am going to create a second list that can be accessed for the
...
//        ResourceValue.initResources(SceneDefaultWidgetID, this);
    ...
    public WidgetAction createRectangularSelectAction() {
        return ActionFactory.createRectangularSelectAction(new DiagramRectangularSelectDecorator(this, DesignerTools.SELECT),
...
//        widget.setTargetAnchor(AnchorFactory.createRectangularAnchor(findWidget(targetNode)));
    ...
    private boolean isSelfLink(ConnectionWidget connection) {
        ...
        return sourceAnchor != null && targetAnchor != null && sourceAnchor.getRelatedWidget() == targetAnchor.getRelatedWidget();
    }
    ...
    private DiagramEngine attachEngine(IDiagram diagram) {
        ...
        if (df != null) {
            ...
            for (DataObject engineObject : engineObjects) {
                InstanceCookie ic = engineObject.getCookie(org.openide.cookies.InstanceCookie.class);
                //if (ic == null) continue;
                Object instance;
                try {
    ...
    public Set<IPresentationElement> getNodesInRectangle(Rectangle sceneSelection) {
        Set< IPresentationElement> retVal = new HashSet< >();
        for (IPresentationElement item : getGraphObjectInRectangle(sceneSelection,
                true, false)) {
            if (isNode(item) == true) {
                retVal.add(item);
    ...
    public void userSelectionSuggested(Set<?> suggestedSelectedObjects, boolean invertSelection) {
        ...
        HashSet< Object> selection = new HashSet<>();
        if (lockedSet.size() > 0) {
