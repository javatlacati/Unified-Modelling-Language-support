...
//            childLayer.setOpaque(true);
    ...
    /**
     * in fact it duplicates getResizeControlPoints() funtionality and need to
     * be changed later
     *
     * @param resizable
     */
        ...
        if (select && !wasSelected) {
            ...
            if (getResizeMode() == RESIZEMODE.PREFERREDBOUNDS) {
                ...
                if (getMinimumSize() == null) {
                    ...
                    if (thebounds != null) {
                        setMinimumSize(new Dimension(thebounds.width - 2 * RESIZE_SIZE, thebounds.height - 2 * RESIZE_SIZE));
                    } else {
                        setMinimumSize(new Dimension(5, 5)); //VERIFY added just to test if it goes better than 0,0
                    }
                }
...
//                }
    ...
    /**
     * Handle property changes from the model element. If the current view is a
     * property change listener the event is forward the view. Otherwise nothing
     * happens.
     *
...
//                scene.removeNodeWithEdges(o);
    ...
    public void removeAttachedEntries(ConnectionWidget connectionWidget) {
        if (connectionWidget != null) {
            ...
            List<Anchor.Entry> removedEntryList = new ArrayList<>();
            if (deps.size() > 0) {
     ...
     * more used for window like resizing, when content is relative to left-top
     * corner
     *
    ...
    /**
     * instead of Widget::getMinimumSize this is dinamic value used in resize to
     * content action and most im[portant for elements without any content or
     * for multiline elements without inner limitations
     *
     * @return miminum size to set in resize to content action, by default the
     * same as in resizing minimum size
     */
    ...
    /**
     * if node contain some content it may be used for example in resizing in
     * general it may be moved to ContainerNode more used with mask like
     * resizing when content is relative to global coordinates
     *
     * @return
     */
    ...
    /**
     * some widgets may be added by some action like context menu and should be
     * removed the same way instead of common delete/cut etc actions (like
     * interaction boundary widget) by default most nodes can be deleted from
     * diagram
     */
