...
//    private WeakReference < UITopComponent > rawTopComponent = null;
    ...
    /**
     * similr to setDrawingArea in 6.0 sometimes it's required to know
     * engine/scene/top component based on diagram
     *
     * @param engine
     */
    ...
    /**
     * similr to getDrawingArea in 6.0 sometimes it's required to know
     * engine/scene/top component based on diagram
     *
     * @param engine
     */
    ...
    public IGraphicExportDetails saveAsGraphic(final String sFilename, int nKind, final double scale) {
        ...
        retVal.setGraphicBoundingRect(new ETRect(scene.getLocation().x, scene.getLocation().y,
                scene.getClientArea().width * scale,
                scene.getClientArea().height * scale));
        ETArrayList<IGraphicMapLocation> locations = new ETArrayList<>();
...
//        }
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                Logger.getLogger("UML").log(Level.SEVERE, "unable to create diagram image file: {0}", e.getMessage());
            }
        });
...
//        if (sceneRect == null) {
//            if (scene != null) {
//                sceneRect = scene.convertSceneToView(new Rectangle());
//            }
//        }
        Point centerPoint;
        if (sceneRect != null) {
            ...
            centerPoint = new Point(50, 50);// VERIFY perhaps better 0,0
        }
    ...
    /**
     * Notifies the diagram to refresh the node graphical object that is
     * associated with the presentation element.
     *
    ...
    /**
     * Search for, and return if found, the presentation element on this diagram
     * that has the specified xmi.id
     *
     * @param sXMIID [in] The xmi.id of the presentation to search for, and find
     * @param pPresentationElement [out,retval] Returns the presentation element
     * on the diagram with the specified xmi.id
     *
     * @return HRESULT
     */
